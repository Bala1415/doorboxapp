...
class DeviceStatusNotifier extends StateNotifier<DeviceStatus> {
  ...
  DeviceStatusNotifier(this._socketService, this._apiService) : super(
    const DeviceStatus(
      name: "Front Porch Box", // This could come from a database/auth later
      isOnline: false,
      isLocked: true,
      wifiStrength: 0,
      batteryLevel: 0,
  ...
  void _initSocketListeners() {
    ...
    _socketService.boxUpdateStream.listen((payload) {
      ...
      if (hwMsg != null) {
        ...
        final isLocked = (rawLock == '1' || rawLock == 1 || rawLock == 'Locked');
        int wifiStrength = state.wifiStrength;
        final wifiRaw = hwMsg['wifi'];
        
        if (wifiRaw is String) {
          final wifiLower = wifiRaw.toLowerCase();
          if (wifiLower == "strong") wifiStrength = 100;
          else if (wifiLower == "medium") wifiStrength = 60;
          else if (wifiLower == "weak") wifiStrength = 30;
          else wifiStrength = parseIntSafe(wifiRaw, state.wifiStrength);
